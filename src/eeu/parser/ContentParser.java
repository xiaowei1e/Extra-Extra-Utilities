...
public class ContentParser {
    ...
    ObjectMap<Class<?>, FieldParser> classParsers = new ObjectMap<>() {{
        ...
        put(ItemStack.class, (type, data) -> {
            if (data.isString() && data.asString().contains("/")) {
                String[] split = data.asString().split("/");
                Item item = locate(ContentType.item, split[0]);
                int amount = Integer.parseInt(split[1]);
                return new ItemStack(item, amount);
            } else return new ItemStack();
        });
        put(LiquidStack.class, (type, data) -> {
            if (data.isString() && data.asString().contains("/")) {
                String[] split = data.asString().split("/");
        ...
        put(PayloadStack.class, (type, data) -> {
            if (data.isString() && data.asString().contains("/")) {
                String[] split = data.asString().split("/");
                ...
                return new PayloadStack(cont == null ? Blocks.router : cont, number);
            } else return null;
        });
        put(FormulaStack.class, (type, data) -> {
            var f = make(FormulaStack.class);
            if (data.isArray()) {
                f.setFormulas(new Seq<>(parser.readValue(Formula[].class, data)));
            }
            return f;
        });
        FieldParser consumeParser = (Class<?> type, JsonValue data) -> {
            var oc = resolve(data.getString("type", ""), type);
            data.remove("type");
            var consume = make(oc);
            readFields(consume, data);
            return consume;
        };
        put(Consume.class, consumeParser);
        put(ConsumeLiquid.class, (type, data) -> {
            if (data.isString() && data.asString().contains("/")) {
                String[] split = data.asString().split("/");
                Liquid liquid = locate(ContentType.liquid, split[0]);
                float amount = Float.parseFloat(split[1]);
        ...
        private <T> T internalRead(Class<T> type, Class elementType, JsonValue jsonData, Class keyType) {
            if (type != null) {
                ...
                /*if (type == ItemStack.class && jsonData.isString() && jsonData.asString().contains("/")) {
                    ...
                    Item item = locate(ContentType.item, split[0]);
                    int amount = Integer.parseInt(split[1]);
                    return (T) new ItemStack(item, amount);
                }*/
                ...
                /*if (jsonData.isString() && jsonData.asString().contains("/")) {
                    ...
                    if (type == LiquidStack.class) {
                        return (T) new LiquidStack(liquid, amount);
                    } else if (type == ConsumeLiquid.class) {
                        return (T) new ConsumeLiquid(liquid, amount);
                    }
                }*/
    ...
    public Content parse(LoadedMod mod, String name, String json, Fi file, ContentType type) throws Exception {
        ...
        String contentName = value.getString("name", "NUL");
        ...
        if (!located) {
            c.minfo.mod = mod;
            if (c instanceof UnlockableContent u) JsonContent.contents.put(mod.name + "-" + contentName, u);
        }
        return c;
    }
    ...
    private <T extends MappableContent> T locate(ContentType type, String name) {
        ...
        T first3 = Vars.content.getByName(type, "extra-utilities-" + name);
        return first != null ? first : (first1 != null ? first1 : (first2 != null ? first2 : first3));
    }
    ...
    private GenericMesh parseMesh(Planet planet, JsonValue data) {
        ...
        return switch (tname) {
            ...
            case "MatMesh" ->
                    new MatMesh(parser.readValue(GenericMesh.class, data.get("mesh")), parser.readValue(Mat3D.class, data.get("mat")));
